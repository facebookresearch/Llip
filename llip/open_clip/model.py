...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None, need_weights=False):
        if need_weights:
            return self.attn(x, x, x, need_weights=need_weights, attn_mask=attn_mask, average_attn_weights=False)
        return self.attn(x, x, x, need_weights=need_weights, attn_mask=attn_mask)[0]

    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None, need_weights=False):
        if need_weights:
            attn, weights = self.attention(self.ln_1(x),
                                           attn_mask=attn_mask,
...
class VisualTransformer(nn.Module):
    def __init__(
            ...
            output_dim: int, act_layer: Callable = nn.GELU, ncls: int = 1, pass_all_tokens: bool = False):
        super().__init__()
        ...
        self.output_dim = output_dim
        self.width = width
        self.pass_all_tokens = pass_all_tokens
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=width,
                               kernel_size=patch_size, stride=patch_size, bias=False)

        scale = width ** -0.5
        self.class_embedding = nn.Parameter(scale * torch.randn(ncls, width))
        self.positional_embedding = nn.Parameter(
            scale * torch.randn(self.grid_size[0] * self.grid_size[1] + ncls, width))
        ...
        if ncls > 1:
            self.proj = nn.Parameter(
                scale * torch.randn(ncls, width, output_dim))
        else:
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = torch.cat(
            [self.class_embedding.to(x.dtype)[None] + torch.zeros(x.shape[0], self.ncls, x.shape[-1], dtype=x.dtype, device=x.device),
        ...
        if self.pass_all_tokens:
            x = self.ln_post(x.squeeze(1))
        else:
            x = self.ln_post(x[:, :self.ncls, :].squeeze(1))

        if self.proj is not None:
            if self.proj.dim() == 2:
                x = x @ self.proj
            elif self.proj.dim() == 3:
                x = x.transpose(0, 1) @ self.proj
                x = x.transpose(0, 1)
    ...
    def get_last_attention(self, x):
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        attn = self.transformer.get_attn(x)
        return attn
...
class CLIP(nn.Module):
    def __init__(
        ...
        if vision_cfg.timm_model_name:
            ...
            self.visual = VisualTransformer(
                ...
                output_dim=embed_dim,
                act_layer=act_layer,
                ncls=vision_cfg.ncls,
                pass_all_tokens=vision_cfg.pass_all_tokens,
            )
    ...
    def get_text_encoding(self, text):
        ...
        h = self.transformer(h, attn_mask=self.attn_mask)
...
class xAttnPred(torch.nn.Module):
    def __init__(self, embed_dim):
        super().__init__()
        scale_out = embed_dim ** -0.5
        self.out_proj = nn.Parameter(
            scale_out * torch.randn(embed_dim, embed_dim))

    def cross_attention(self, K, Q, V, weight_scale, out_proj):
        attn = (torch.einsum('vhnd,thd->vthn', K, Q) /
                weight_scale).softmax(-1)
        zv = torch.einsum('vthn,vhnd->vthd', attn,
                          V).reshape(K.shape[0], Q.shape[0], -1)
        zv = zv @ out_proj
        return zv
...
class CMP(CLIP):
    def __init__(self, embed_dim, num_heads, use_norm=False, *args, **kwargs):
        super().__init__(embed_dim, *args, **kwargs)
        self.visual.proj = None
        ncls = self.visual.ncls
        self.num_heads = num_heads

        scale_visual = self.visual.width ** -0.5
        if self.visual.pass_all_tokens:
            num_proj = self.visual.positional_embedding.size(0)
        else:
            num_proj = ncls
        self.v_proj = nn.Parameter(
            scale_visual * torch.randn(num_proj, self.visual.width, embed_dim))
        self.k_proj = nn.Parameter(
            scale_visual * torch.randn(num_proj, self.visual.width, embed_dim))

        scale_text = self.transformer.width ** -0.5
        self.q_proj = nn.Parameter(
            scale_text * torch.randn(self.transformer.width, embed_dim))
    ...
    def encode_image(self, image):
        ...
        K = self.K_norm(K)
        V = self.V_norm(V)
        K = K.reshape(N, B, self.num_heads, C //
                      self.num_heads).permute(1, 2, 0, 3)  # [B, num_heads, N, D]
        V = V.reshape(N, B, self.num_heads, C //
                      self.num_heads).permute(1, 2, 0, 3)
        return K, V
    ...
    def forward(self, image, text, clamp_logit_scale_to=None):
        ...
        '''
        image_output = self.encode_image(image)
        text_output = self.encode_text(text)

        if clamp_logit_scale_to is not None:
            with torch.no_grad():
                self.logit_scale.data.clamp_(0, clamp_logit_scale_to)

        return image_output, text_output, self.pred, self.logit_scale.exp(), self.logit_bias

    def image_prediction(self, image, text, weight_scale):
        K, V = self.encode_image(image)
        Q, _ = self.encode_text(text)

        return self.pred(K, Q, V, weight_scale)
