...
def parse_start_end(shards):
    start, end = os.path.basename(shards).split(
        "{")[1].split("}")[0].split("..")
...
def search_config(config_name):
    ...
    all_configs = {}
    config_dir = os.path.join(project_dir, 'run_configs')
    for code in os.listdir(config_dir):
        if code.endswith(".py"):
            print(f"searching config in {code}")
            module = importlib.import_module(
                f'llip.run_configs.{code[:-3]}')
